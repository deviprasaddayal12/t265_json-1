...
class T265Json():
    ...
    def __init__(self):
        ...
        self.odometry_subscriber = rospy.Subscriber(
            '/camera/odom/sample', Odometry, self.callback_t265)
        print("callback for JSON_from_phone")
        self.JSON_subscriber = rospy.Subscriber(
            '/JSON_from_phone', JSON, self.callback_JSON)
        self.Trigger_publisher = rospy.Publisher(
            '/android_call_trigger', Bool, queue_size=10)
        self.Distance_Trigger_publisher = rospy.Publisher(
            '/distance_complete_trigger', Bool, queue_size=10)

        self.Mappingstatus_subscriber = rospy.Subscriber(
            "/MAPPING_STATUS", Bool, self.mappingstatus_callback)
    ...
    def callback_t265(self, data):
        ...
        if(self.MAPPING_STATUS):
            ...
            if (self.TIME_BASED_TRIGGER):
                ...
                elif (math.hypot((last_x - X), (last_y - Y)) > 0.1):

                    print('Elif Condition')
                    self.last_odom = data
                    # self.append_csv(data)
                    self.Distance_Trigger_publisher.publish(True)
                    print((last_x - X), (last_y - Y))
                    return True

    def callback_JSON(self, data):
        self.MAP_FILE_NAME = data.MAP_NAME + \
            "-" + str(self.current_time) + '.csv'
    ...
    def send_JSON(self, data):
        JSON_OBJECT = {"X": str(round(data.pose.pose.position.x, 4)), "Y": str(
            round(data.pose.pose.position.y, 4)), "Timestamp": self.current_time}
    ...
    def create_csv(self, data):
        ...
        with open(Package_Path+'/output/'+self.MAP_FILE_NAME, mode='w') as file:
            ...
            user_writer.writerow(
                ['Time-based Trigger', int(data.TIME_BASED_TRIGGER)])
            user_writer.writerow(
                ['Distance-based Trigger', int(data.DISTANCE_BASED_TRIGGER)])
    ...
    def append_csv(self, data):
        ...
        fields = [str(round(data.pose.pose.position.x, 4)), str(
            round(data.pose.pose.position.y, 4)), self.current_time]
    ...
    def append(self, data):
        ...
        with open('user_data.csv', 'a') as file:
            ...
            writer.writeheader()
            writer.writerow({'X': str(round(data.pose.pose.position.x, 4)), 'Y': str(
                round(data.pose.pose.position.y, 4)), "Timestamp": self.current_time})
